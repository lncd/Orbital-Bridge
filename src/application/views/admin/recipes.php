<div class="page-header">

	<h1>
		<i class="icon-cogs"></i> Administration
	</h1>

</div>

<ul class="nav nav-pills">
	<li><a href="<?php echo site_url('admin'); ?>"><i class="icon-dashboard"></i> Overview</a></li>
	<li><a href="<?php echo site_url('admin/applications'); ?>"><i class="icon-list-alt"></i> Applications</a></li>
	<li class="active"><a href="<?php echo site_url('admin/recipes'); ?>"><i class="icon-book"></i> Recipes</a></li>
	<li><a href="<?php echo site_url('admin/pages'); ?>"><i class="icon-file"></i> Pages</a></li>
	<li><a href="<?php echo site_url('admin/categories'); ?>"><i class="icon-file"></i> Categories</a></li>
</ul>

<hr>
		
<p class="lead">There are no recipes stored in Orbital.</p>
		
<p><a class="btn btn-success disabled"><i class="icon-plus"></i> Create Recipe</a></p>