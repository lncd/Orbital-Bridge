<?php
		...
		if ($testsdr->sac_status == 200)
		{
			...
			foreach ($testsdr->sac_workspaces as $workspace)
			{
				...
				foreach ($collections as $collection)
				{
					...
					$mediation = "false";
					if ($collection->sac_mediation == true)
					{
						$mediation = "true";
					}
	...
	if (true)
	{
		...
		if ($testdr->sac_status == 201)
		{
			...
			print " - Generator: " . $testdr->sac_generator . " (" . $testdr->sac_generator_uri . ")\n";
			print " - User agent: " . $testdr->sac_useragent . "\n";
			if (!empty($testdr->sac_noOp))
			{
				print " - noOp: " . $testdr->sac_noOp . "\n";
			}
		}
	}
	...
	if (false)
	{
		...
		$testdr = $testsac->deposit($testdepositurl, $testuser, $testpw, $testobo, $testfile, "METS/SWAP");
		print "Received HTTP status code: " . $testdr->sac_status . " (" . $testdr->sac_statusmessage . ")\n";
